...
mailx -s "RMAN Level 1 Backup Completed [$ORACLE_SID]" you@example.com < "$LOGFILE"
...
cd "$LOG_DIR" || exit 1
OLDER_LOGS=$(ls -1t rman_backup_level1_*.log | tail -n +61)
...
if [ -n "$OLDER_LOGS" ]; then
  ...
  echo "No Level 1 logs to archive." >> "$LOGFILE"
fi